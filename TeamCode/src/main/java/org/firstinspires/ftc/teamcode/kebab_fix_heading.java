...
public class kebab_fix_heading {
    ...
    public static kebab_fix_heading fromHardwareMap(
        ...
        stanga.setZeroPowerBehavior(
                DcMotor.ZeroPowerBehavior.BRAKE);

        dreapta.setZeroPowerBehavior(
                DcMotor.ZeroPowerBehavior.BRAKE);
    ...
    public void updateXY(
        ...
        double dx = targetX_mm -
                pose.getX(DistanceUnit.MM);

        double dy = targetY_mm -
    ...
    public void updateHeading(
        ...
        // Cea mai scurta diferenta unghiulara
        double error = normalizeAngle(
                targetHeadingDeg - current);
        ...
        if (Math.abs(error) > KS_MIN_ERROR_DEG) {
            ...
            power = raw
                    + Math.copySign(HEADING_KS, raw);

        } else {

            // Aproape de tinta
            // Fara kS pentru a reduce oscilatia
            power = raw;
        }
    ...
    private static double applyMinPower(
            double power,
            double minPower) {
